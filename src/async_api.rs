...
pub async fn generate_pdf_async(markdown: &str, font: &str, font_size: f32) -> Result<Vec<u8>> {
...
/// optimized bytes.
pub async fn optimize_pdf_async(path: &str, settings: OptimizationSettings) -> Result<Vec<u8>> {
    let bytes = tokio::fs::read(path).await?;
...
mod tests {
    ...
    async fn test_load_pdf_async_roundtrip() {
        ...
        assert!(
            ...
            "Should extract text from async-loaded PDF: {}",
            text
        );
    ...
    async fn test_validate_pdf_async() {
        ...
        let result = validate_pdf_async(&tmp_path.to_string_lossy())
            .await
            .unwrap();
        assert!(
            result.valid,
            "Generated PDF should be structurally valid: {:?}",
            result.errors
        );
