...
fn parse_bold_italic(text: &str) -> Vec<TextSegment> {
    ...
    let consider =
        |best: &mut Option<(usize, usize, Kind, String)>, caps: regex::Captures, kind: Kind| {
...
pub fn parse_markdown_with_hook(
    ...
    while i < lines.len() {
        ...
        if in_math_block {
            if trimmed == "$$" {
                elements.push(Element::MathBlock {
                    expression: math_buf.clone(),
                });
                math_buf.clear();
                in_math_block = false;
            } else {
        ...
        if trimmed.starts_with('|') && trimmed.ends_with('|') {
            ...
            if is_separator {
                let alignments: Vec<TableAlignment> =
                    cells.iter().map(|c| parse_cell_alignment(c)).collect();
                elements.push(Element::TableRow {
                    cells,
                    is_separator: true,
                    alignments,
                });
                ...
                let cells: Vec<String> = cells
                    .into_iter()
                    .map(|c| strip_inline_formatting(&c))
                    .collect();
        ...
        if let Some(dot_pos) = trimmed.find(". ") {
            ...
            if !num_part.is_empty() && num_part.chars().all(|c| c.is_ascii_digit()) {
                ...
                elements.push(Element::OrderedListItem {
                    number,
                    text,
                    depth,
                });
...
mod tests {
    ...
    fn test_parse_strikethrough() {
        ...
        assert_eq!(
            strip_inline_formatting("keep ~~this~~ text"),
            "keep this text"
        );
    }
    ...
    fn test_parse_mixed_inline_styles_left_to_right() {
        let segments =
            parse_inline_formatting("Emphasis: **bold**, *italic*, ***both***, `code`, ~~strike~~");
        assert!(
            segments
                .iter()
                .any(|s| matches!(s, TextSegment::Bold(t) if t == "bold"))
        );
        assert!(
            segments
                .iter()
                .any(|s| matches!(s, TextSegment::Italic(t) if t == "italic"))
        );
        assert!(
            segments
                .iter()
        ...
        assert!(
            segments
                .iter()
                .any(|s| matches!(s, TextSegment::Code(t) if t == "code"))
        );
        assert!(
            segments
                .iter()
                .any(|s| matches!(s, TextSegment::Strikethrough(t) if t == "strike"))
        );
    ...
    fn test_parse_footnote_reference_in_paragraph() {
        ...
        match &elements[0] {
            Element::Paragraph { text } => {
                assert!(
                    text.contains("(1)"),
                    "Footnote ref should be converted to (1), got: {}",
                    text
                );
                assert!(
                    !text.contains("[^1]"),
                    "Raw footnote ref should be stripped"
                );
            }
...
<!-- pagenumber:arabic -->\n";
        ...
        assert!(
            elements
                .iter()
                .any(|e| matches!(e, Element::RunningHeaderMode { enabled: true }))
        );
        ...
        assert!(
            elements
                .iter()
                .any(|e| matches!(e, Element::CitationDef { key, .. } if key == "smith2020"))
        );
    ...
    fn test_parse_columns_directive() {
        ...
        assert!(
            elements
                .iter()
                .any(|e| matches!(e, Element::Columns { count: 2 }))
        );
        assert!(
            elements
                .iter()
                .any(|e| matches!(e, Element::Columns { count: 1 }))
        );
    }
        ...
        match &elements[0] {
            Element::RichParagraph { segments } => {
                assert!(
                    segments
                        .iter()
                        .any(|s| matches!(s, TextSegment::MathInline(expr) if expr == "E = mc^2"))
                );
            }
